...
Una passata inizia con un impulso di testa, seguito da un tratto di silenzio (\emph{lead}) di cinque secondi, che lascia assestare l'ambiente di misura prima che lo sweep abbia inizio.
...
Il metodo di avvio \texttt{trigger()} del transport ignora le richieste che arrivano mentre una passata \`e gi\`a in corso, cos\`i il transport mantiene una passata alla volta e la sua struttura resta prevedibile per uno strumento di misura.
Il plugin espone \texttt{Loop} come unico comando di transport, e a esso affida l'avvio e la ripetizione delle passate.
Un pulsante di avvio istantaneo, provato in fase di collaudo, consegna in alcuni host la propria pressione come un singolo valore, quello finale a zero, e per questo il comando \texttt{Loop} a valore stabile governa il transport in modo affidabile su ogni host.
Il metodo \texttt{trigger()} resta nel motore SDK-free, dove il collaudo automatico lo verifica e la fase di ricevitore potr\`a riusarlo.
