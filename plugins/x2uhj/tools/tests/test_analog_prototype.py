...
def test_phase_endpoints():
    ...
    assert analog_allpass_phase(f0, 0.7071, np.array([1e7]))[0] < -2*np.pi + 1e-2  # 1e7 Hz approximates ω→∞, hence the loose bound

def test_phase_is_monotonic_decreasing():
    freqs = np.geomspace(1.0, 1e6, 4096)
    ph = analog_allpass_phase(1000.0, 0.7071, freqs)
    assert np.all(np.diff(ph) <= 1e-9)  # float64 noise-floor allowance, not a physical tolerance
